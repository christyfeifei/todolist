...
def user_todos_api(request,id):
    ...
    json_data = json.dumps({'datetime':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'result':result,'user':user, 'data':todo_list},ensure_ascii=False)
...
def todo_api (request,id):
    ...
    json_data = json.dumps({'datetime':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'result':result,'data':todo_data},ensure_ascii=False)
